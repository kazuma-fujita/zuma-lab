...
最後に provider を使用する為に、使用する Widget を ProviderScope でくくります。
...
また、ChangeNotifierProvider の ref オブジェクト利用して他の provider にアクセス出来るようです。
...
ただし本来、状態を保持するオブジェクトは代入を許可せず immutable(不変)に操作されるべきです。
...
ポイントは `context.read(todoProvider).updateTodo(todo.id, _title);` と `context.read(todoProvider).createTodo(_title);` の処理で、先程作成した ViewModel の `updateTodo` と `createTodo` メソッドを呼び出しています。
...
`context.read` で Provider で保持している ViewModel の関数呼び出し、ViewModel で UI の状態を保持した todoList の変更、`ChangeNotifier`で変更を通知、`useProvider` で状態変更を検知して ListView.builder をリビルドをします。
